...
ProductRouter.post("/create-product",requireSignIn,isAdmin, formidable(), async (req, res) => {
    try {
      ...
      const products = new productsses({
        ...req.fields,
        slug: slugify(name),
        createdBy: req.user._id, // Assign the authenticated admin's ID
      });
...
ProductRouter.get("/get-admin-products", requireSignIn, isAdmin, async (req, res) => {
  try {
    const products = await productsses
      .find({ createdBy: req.user._id }) // Filter by the authenticated admin
      .populate("category")
      .select("-photo")
      .sort({ createdAt: -1 });
    res.status(200).send({
    ...
    res.status(500).send({
      success: false,
      message: "Error in getting admin products",
      error: error.message,
    });
  }
});
