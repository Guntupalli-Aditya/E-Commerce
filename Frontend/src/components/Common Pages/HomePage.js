...
const HomePage = () => {
  ...
  const getAllProducts = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(`https://e-commerce-backend-928q.onrender.com/ProductRoutes/product-list`, {
        params: { page }
      });
      setLoading(false);
  ...
  const getTotal = async () => {
    try {
      const { data } = await axios.get("https://e-commerce-backend-928q.onrender.com/ProductRoutes/product-count");
      setTotal(data?.total);
    } catch (error) {
      console.log(error);
    }
  ...
  const filterProduct = async () => {
    try {
      const { data } = await axios.post("https://e-commerce-backend-928q.onrender.com/ProductRoutes/product-filters", {
        checked,
        radio,
      });
      setProducts(data.products);
