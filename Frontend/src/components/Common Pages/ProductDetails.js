...
const ProductDetails = () => {
  ...
  const getProduct = async () => {
    try {
      const { data } = await axios.get(
        `https://e-commerce-backend-928q.onrender.com/ProductRoutes/get-product/${params.slug}`
      );
  ...
  const getSimilarProduct = async (pid, cid) => {
    try {
      const { data } = await axios.get(
        `https://e-commerce-backend-928q.onrender.com/ProductRoutes/related-product/${pid}/${cid}`
      );
